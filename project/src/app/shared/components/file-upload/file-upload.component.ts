...
export class FileUploadComponent {
    ...
    if (files && files.length > 0) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (this.isValidFileType(file)) {
          ...
          this.errorMessage = 'Пожалуйста, выберите только файлы формата JSON или SARIF';
          this.selectedFiles = [];
          return;
        }
      }
      ...
      const promise = new Promise((resolve, reject) => {
        ...
        reader.onload = (e: any) => {
          try {
            ...
            reject(new Error(`Неверный формат файла ${file.name}. Файл должен быть в формате JSON.`));
          }
        };
        reader.onerror = () => {
  ...
  private uploadToDatabase(data: any): void {
    try {
      ...
      push(uploadsRef, uploadData).then(() => {
        ...
        this.handleUploadError('Ошибка при загрузке данных в базу: ' + error.message);
      });
    } catch (error: any) {
      this.handleUploadError('Ошибка при подключении к базе данных: ' + error.message);
    }
  }
